<script setup lang="ts">
    ...
    // 触底加载
    let touchbottom:Ref<null|HTMLButtonElement> = ref(null);
    function openIo(){
    ...
    <div>
        ...
        <ul>
            ...
            <li v-for="(item,index) in showData" :key="item.id">
                ...
                <button v-if="!topic" @blur="detailsChange()">
                    <LikeIcon v-if="item.id==detailsIndex" :ids="item.id" />
                    <svg v-else @click="detailsChange(item.id)" class="icon" aria-hidden="true">
